//Assignment 1 - SG2658 NJIT CLOUD CS643
package com.aws.ec2;
...
public class ImageRekognition {
    public static void main(String[] args) throws IOException, JMSException, InterruptedException {
        ...
        try {
            ...
            // Create an Amazon SQS FIFO queue named SG2658.fifo, if it doesn't already
            // exist
            if (!client.queueExists("SG2658.fifo")) {
                Map<String, String> attributes = new HashMap<String, String>();
                attributes.put("FifoQueue", "true");
                attributes.put("ContentBasedDeduplication", "true");
                client.createQueue(new CreateQueueRequest().withQueueName("SG2658.fifo").withAttributes(attributes));
            }
            // Create the nontransacted session with AUTO_ACKNOWLEDGE mode
            Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
            // Create a queue identity and specify the queue name to the session
            Queue queue = session.createQueue("SG2658.fifo");

            // Create a producer for the 'SG2658'
